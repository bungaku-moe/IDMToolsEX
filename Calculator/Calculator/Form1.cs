...
public partial class SimpleCalculator : Form
{
    ...
    protected override void OnFormClosing(FormClosingEventArgs e)
    {
        ...
        WindowState = FormWindowState.Minimized; // Hide the form (or use this.WindowState = FormWindowState.Minimized)
    }
    ...
    private void ButtonNumbers_Click(object sender, EventArgs e)
    {
        ...
        if (textMainDisplay.TextLength < 13 + Convert.ToInt32(textMainDisplay.Text.Contains(btnDot.Text)) +
    ...
    {
        var btnDot = (Button)sender;
        //MessageBox.Show("Button Dot clicked");
        if (textMainDisplay.Text.Contains(btnDot.Text) == false) textMainDisplay.Text += btnDot.Text;
    }
    ...
    {
        EnableOperationKeys(sender, e);
        if (textFormulaDisplay.Text.Contains("=")) textFormulaDisplay.Clear();
        textMainDisplay.Text = "0";
    }
    ...
    private void ButtonBasicOperation_Click(object sender, EventArgs e)
    {
        ...
        var mainDisplayValue = double.Parse(textMainDisplay.Text);
        ...
        if (textMainDisplay.Text != result.ToString(precisionFormat) || textMainDisplay.Text == "0")
        {
            ...
            if (textFormulaDisplay.Text != string.Empty && !textFormulaDisplay.Text.Contains('='))
            {
                ...
                if (specialOperation != string.Empty)
                {
                    ...
                    {
                        var displayValue = operand[0] != double.MaxValue ? operand[0] : 0.0;
                        textFormulaDisplay.Text += $" ({displayValue}) × {result}% = ";
                    }
                    else
                    {
                        textFormulaDisplay.Text +=
                            " " + specialOperation + "(" + result.ToString(precisionFormat) + ") =";
                    }
                }
                else
                {
    ...
    {
        if (double.TryParse(textMainDisplay.Text, out var standardValue))
    ...
    private void SimpleCalculator_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.KeyCode == Keys.Oemplus || e.KeyCode == Keys.Add)
            ...
            btnDot.PerformClick();
        else if (e.KeyCode == Keys.Delete) btnClearEntry.PerformClick();
    }
    ...
    {
        if (keyData == Keys.Enter && ActiveControl != null)
            if (ActiveControl.Handle != btnEqual.Handle)
                btnEqual.Select();
